...
class Links extends Module {
	...
	public function register_routes() {
		register_rest_route( 'dubco/v1', '/links', [
			'methods'             => WP_REST_Server::CREATABLE,
			'callback'            => [ $this, 'create_link' ],
			'permission_callback' => [ $this, 'permissions_check' ],
		] );

		register_rest_route( 'dubco/v1', '/links/(?P<id>[a-zA-Z0-9]+)', [
	...
	public function create_link( $request ) {
		...
		$response      = $api_client->request( 'POST', '/links', [
	...
	public function update_link( $request ) {
		...
		$api_client    = ApiClient::get_instance();
		$response      = $api_client->request( 'PATCH', '/links/' . $id , [
			'key' => $params['key'],
		] );
